...
# Using pbkdf2_sha256 because bcrypt has version compatibility issues on Windows
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    access_token = create_access_token(
        data={"sub": user["username"], "role": user.get("role", "public")}, expires_delta=access_token_expires
    )
    return {"access_token": access_token, "token_type": "bearer"}
...
async def create_complaint(complaint: ComplaintCreate, current_user: dict = Depends(get_current_user)):
    ...
    complaint_dict["user_id"] = current_user["username"]
    complaint_dict["timestamp"] = datetime.utcnow()
    complaint_dict["status"] = "open"
    
    result = complaints_collection.insert_one(complaint_dict)
    return {"message": "Complaint submitted successfully", "id": str(result.inserted_id)}
...
async def get_complaints(current_user: dict = Depends(get_current_user)):
    ...
    complaints = []
    cursor = complaints_collection.find().sort("timestamp", -1)
    for c in cursor:
        c["id"] = str(c["_id"])
        del c["_id"]
        complaints.append(c)
